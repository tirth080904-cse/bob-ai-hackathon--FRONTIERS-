...
When a major shipping corridor goes down â€” a geopolitical blockage on the Suez
Canal, a port closure at JNPT, a storm forcing vessels off the Strait of Malacca
â€” the impact is never limited to one shipment. The same corridor carries dozens
...
correctly changes the urgency ranking of questions 2 and 3 â€” a vaccine shipment
...
|---|---|---|
| **Operations Director** (`ops.director`) | Situational overview â€” how many shipments are exposed, what is the severity, what is the recommended next action | A single summary that aggregates across route, fleet, and cold-chain state without switching between systems |
| **Fleet Commander** (`captain.india`) | Which idle assets are available, which are cargo-compatible, which can be redeployed today | A real-time idle/in-use breakdown that is already filtered by the cargo type of the affected shipment â€” not a raw fleet list |
...
tracked â€” vaccines (2â€“8 Â°C), frozen seafood (âˆ’25 to âˆ’15 Â°C), fresh produce
(2â€“6 Â°C), industrial chemicals (15â€“25 Â°C), and blood samples (2â€“6 Â°C) â€” each
...
- The alternative routes change â€” Lombok Strait and Sunda Strait replace Cape of
  Good Hope; Singapore Changi replaces Dubai Hub for perishable cargo.
- The fleet compatibility matrix changes â€” idle assets at Singapore Anchorage
  (VSL-001) become relevant where they were not before.
- The cargo priority ranking changes â€” which shipment has the most urgent need
...
synthesise a recommendation â€” all before the situation changes again.
...
| Gap | Evidence in `src/` |
|---|---|
| No cargo-type-aware route filtering | `src/data/shipments.py` â€” every shipment has a `cargo_type` field; `ALTERNATIVE_ROUTES` has separate keys for `perishable`, `hazardous`, and `all` |
| No idle/affected shipment cross-reference | `src/modules/fleet_optimizer.py` â€” `match_assets_to_shipments()` filters idle assets by `suitable_for` âˆ© `cargo_type` |
| No severity grading for temperature breaches | `src/modules/cold_chain_monitor.py` â€” explicit three-tier classification with a 3 Â°C `WARNING_MARGIN` separating WARNING from CRITICAL |
| No single view connecting all four domains | `src/dashboard.html` â€” Operations Dashboard screen renders affected shipments, idle fleet count, cold-chain alert count, and priority next actions in one page; any change to the disruption selector re-renders all four simultaneously |
